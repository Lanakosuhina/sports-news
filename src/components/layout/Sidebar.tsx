      ...
      {popularArticles.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="bg-slate-900 text-white px-4 py-3 flex items-center gap-2">
            ...
            <h3 className="font-semibold">Популярное</h3>
          </div>
          <div className="divide-y">
            {popularArticles.map((article, index) => (
              <Link
                ...
                className="flex gap-3 p-4 hover:bg-slate-50 transition group"
              >
                <span className="text-2xl font-bold text-slate-200 group-hover:text-orange-500 transition w-8 flex-shrink-0">
            ...
            {upcomingMatches.map((match) => (
              <div key={match.id} className="p-4 hover:bg-slate-50 transition">
                <div className="text-xs text-slate-500 mb-2 flex items-center justify-between">
                  <span className="font-medium text-orange-500">{match.league.name}</span>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          {standings.length > 8 && (
            <div className="px-4 py-2 border-t border-slate-100 text-center">
              <Link href="/standings" className="text-sm text-orange-500 hover:text-orange-600 font-medium">
                Показать всё
              </Link>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => (
              <Link
                key={tag.id}
                href={`/tag/${tag.slug}`}
                className="px-3 py-1.5 bg-slate-100 hover:bg-orange-500 hover:text-white rounded-full text-sm transition font-medium text-slate-700"
              >
                #{tag.name}
              </Link>
            ))}
